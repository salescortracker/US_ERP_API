...
namespace Usine_Core.Controllers.Purchases
{
    ...
    public class PurCostReportsController : ControllerBase
    {
        private readonly IHostingEnvironment ho;
        public PurCostReportsController(IHostingEnvironment hostingEnvironment)
        {
        ...
        [HttpPost]
        [Authorize]
        [Route("api/PurCostReports/PurRepSupplierDayBook")]
        public PurSupplierDayBookTotal PurRepSupplierDayBook([FromBody] GeneralInformation inf)
        {
            if(ac.screenCheck(inf.usr,2,11,4,0))
            {
                try
                {
                    ...
                    //   List<PurSupplierDayBookInfo> det1 = new List<PurSupplierDayBookInfo>();
                    var det1 = (from a in db.PurPurchasesUni.Where(a => a.Dat >= dat1 && a.Dat < dat2 && a.BranchId == inf.usr.bCode && a.CustomerCode == inf.usr.cCode)
                    ...
                    var det2 = (from a in db.PurPurchaseReturnsUni.Where(a => a.Dat >= dat1 && a.Dat < dat2 && a.BranchId == inf.usr.bCode && a.CustomerCode == inf.usr.cCode)
                                select new PurSupplierDayBookInfo
                                {
                                    seq = a.Seq,
                                    dat = a.Dat.Value,
                    ...
                    var det3 = (from a in db.PartyCreditDebitNotes.Where(a => a.TransactionType == "PCR" && a.Dat >= dat1 && a.Dat < dat2 && a.BranchId == inf.usr.bCode && a.CustomerCode == inf.usr.cCode)
                    ...
                    var det4 = (from a in db.PartyPaymentsuni.Where(a => a.VoucherType == "PUR_PAY" && a.Dat >= dat1 && a.Dat < dat2 && a.BranchId == inf.usr.bCode && a.CustomerCode == inf.usr.cCode)
                    ...
                    msg = ep.pdfLandscapeConversion(filename, "Purchase Daybook on " + inf.frmDate , inf.usr, dt, titles, widths, aligns, true);
                    if (msg == "OK")
                    {
                        tot.pdfFile = fname;
                    }
                    ...
                    msg = "";
                    msg = ep.makeExcelConversion(filename1, "Purchase Daybook on " + inf.frmDate, inf.usr, dt, titles, widths, aligns, true);
                    if (msg == "OK")
                    {
        ...
        [HttpPost]
        [Authorize]
        [Route("api/PurCostReports/PurRepSupplierLedger")]
        public PurSupplierLedgerTotal PurRepSupplierLedger([FromBody] GeneralInformation inf)
        {
            if (ac.screenCheck(inf.usr, 2, 11, 4, 0))
            {
                General gg = new General();
                ...
                msg = ep.pdfLandscapeConversion(filename, "Supplier Ledger from " + inf.frmDate + " to " + inf.toDate, inf.usr, dt, titles, widths, aligns, false);
                if (msg == "OK")
                {
                    tot.pdfFile = fname;
                }
                ...
                string filename1 = ho.WebRootPath + "\\Reps\\" + fname1;
                msg = "";
                msg = ep.makeExcelConversion(filename1, "Supplier Ledger from " + inf.frmDate + " to " + inf.toDate, inf.usr, dt, titles, widths, aligns, false);
                if (msg == "OK")
                {
                    tot.excelFile = fname1;
                }
        ...
        [HttpPost]
        [Authorize]
        [Route("api/PurCostReports/PurRepSupplierSnapShot")]
        public PurSupplierLedgerTotal PurRepSupplierSnapShot([FromBody] UserInfo usr)
        {
            if (ac.screenCheck(usr, 2, 11, 4, 0))
            {
                ...
                quer = quer + " (select * from partydetails where partyType = 'SUP' and customerCode = " + usr.cCode + ")b where a.partyId = b.recordId order by partyname";
        ...
        [HttpPost]
        [Authorize]
        [Route("api/PurCostReports/PurRepSupplierAgingDetails")]
        public PurSupplierLedgerTotal PurRepSupplierAgingDetails([FromBody] UserInfo usr)
        {
            if (ac.screenCheck(usr, 2, 11, 4, 0))
            {
                General gg = new General();
                ...
                quer = quer + " (select transactionId, partyid, pendingAmount from partyTransactions where transactionType= 'PUR' and";
                ...
                quer = quer + " (select * from partyDetails where partytype = 'SUP' and customerCode = " + usr.cCode + ")b where a.partyId = b.recordId";
        ...
        [HttpPost]
        [Authorize]
        [Route("api/PurCostReports/PurRepSupplierClosingBalances")]
        public PurSupplierLedgerTotal PurRepSupplierClosingBalances([FromBody] UserInfo usr)
        {
            if (ac.screenCheck(usr, 2, 11, 4, 0))
            {
                PurSupplierLedgerTotal tot = new PurSupplierLedgerTotal();
                ...
                var lst2 = db.PartyDetails.Where(a => a.PartyType == "SUP" && a.CustomerCode == usr.cCode).ToList();
               var details = (from a in lst1
                ...
                msg = ep.pdfConversion(filename, "Supplier Closing Balances as on " + gg.strDateTime(dats), usr, dt, titles, widths, aligns, true);
                if (msg == "OK")
                {
                    tot.pdfFile = fname;
                }
                ...
                string filename1 = ho.WebRootPath + "\\Reps\\" + fname1;
                msg = "";
                msg = ep.makeExcelConversion(filename1, "Supplier Closing Balances as on " + gg.strDateTime(dats), usr, dt, titles, widths, aligns, true);
                if (msg == "OK")
                {
                    tot.excelFile = fname1;
                }
